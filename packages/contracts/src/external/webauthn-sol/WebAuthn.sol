...
import {Base64} from "openzeppelin-ext/contracts/utils/Base64.sol";
...
///         Changed the verification order to estimate ERC-4337 gas limit more prec
///
...
/// @author a42x Modified from https://github.com/base-org/webauthn-sol
library WebAuthn {
        ...
        returns (bool)
    {
        ...
        // Moved the validation to the end of the function to calculate precise gas to be used
        ...
        bool isValidChallenge = keccak256(bytes(actualChallenge)) == keccak256(expectedChallenge);
        ...
        if (success && valid) return abi.decode(ret, (uint256)) == 1 && isValidChallenge;

        return FCL_ecdsa.ecdsa_verify(messageHash, webAuthnAuth.r, webAuthnAuth.s, x, y) && isValidChallenge;
    }
}
